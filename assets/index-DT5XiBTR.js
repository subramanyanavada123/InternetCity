import{a as Q,t as b,e as J,d as X,s as Z,b as ee}from"./main-CpDT_o6_.js";import{s as I}from"./sfx-DMPbFRfs.js";import"./modulepreload-polyfill-B5Qt9EMX.js";const te=[{id:"hospital",emoji:"🚑",name:"Hospital",priority:"CRITICAL",needs:40,color:"#ff6b6b"},{id:"fire",emoji:"🚒",name:"Fire Station",priority:"CRITICAL",needs:30,color:"#ff9f43"},{id:"school",emoji:"🏫",name:"School",priority:"HIGH",needs:25,color:"#ffd700"},{id:"news",emoji:"📡",name:"News Station",priority:"HIGH",needs:20,color:"#74b9ff"},{id:"residential",emoji:"🏠",name:"Residential",priority:"MEDIUM",needs:35,color:"#a29bfe"},{id:"mall",emoji:"🛒",name:"Mall",priority:"MEDIUM",needs:25,color:"#fd79a8"},{id:"gaming",emoji:"🎮",name:"Gaming Server",priority:"LOW",needs:50,color:"#55efc4"},{id:"streaming",emoji:"📺",name:"Streaming",priority:"LOW",needs:45,color:"#636e72"}],oe={CRITICAL:4,HIGH:3,MEDIUM:2,LOW:1},E=[{label:"Normal Operations",pool:150,surgeIds:[]},{label:"SURGE EVENT",pool:150,surgeIds:["hospital","fire"]},{label:"BLACKOUT",pool:80,surgeIds:[]}];function ne(){if(document.getElementById("m11-styles"))return;const a=document.createElement("style");a.id="m11-styles",a.textContent=`
  ...
  `,document.head.appendChild(a)}function S(a,x){return a>=x?1:a>=x*.7?.7:.3}function ie(a){return a>=1?"😊":a>=.7?"😐":"😤"}function re(a,x){let g=0;for(const i of a){const h=S(x[i.id]||0,i.needs);i.priority==="CRITICAL"&&(x[i.id]||0)===0&&(g-=50),g+=h*100*oe[i.priority]}return Math.round(g)}function se(a,x,g){ne();const i=document.createElement("div");i.style.cssText=`
    position:fixed;inset:0;background:#1a0800;
    font-family:'Space Mono',monospace,sans-serif;
    overflow-y:auto;z-index:10;color:#fff;
  `,a.appendChild(i);const h=Q(i,{color:"#ffb347"}),C=document.createElement("button");C.style.cssText=`
  ...
  `,C.textContent=b("btn.back"),C.addEventListener("click",()=>{G(),g(0,0)}),i.appendChild(C);let c=0,p=0,v=30,m=null,M=!1,f={},u=[];function Y(){const t=E[c];u=te.map(o=>({...o,needs:t.surgeIds.includes(o.id)?o.needs*2:o.needs})),f={},u.forEach(o=>{f[o.id]=0})}function B(){return E[c].pool}function A(){return Object.values(f).reduce((t,o)=>t+o,0)}const T=document.createElement("div");T.style.cssText="text-align:center;padding:56px 16px 6px;",T.innerHTML=`
  ...
  `,d.textContent="CONFIRM ALLOCATION",d.addEventListener("mouseenter",()=>{d.style.transform="scale(1.04)",d.style.boxShadow="0 6px 28px rgba(255,160,50,0.6)"}),d.addEventListener("mouseleave",()=>{d.style.transform="scale(1)",d.style.boxShadow="0 4px 20px rgba(255,160,50,0.4)"}),d.addEventListener("click",U),i.appendChild(d);function P(){$.innerHTML="";const t=E[c],o={CRITICAL:"#ff6b6b",HIGH:"#ffd700",MEDIUM:"#a29bfe",LOW:"#55efc4"};u.forEach(e=>{const n=document.createElement("div");n.className="m11-bidder-card",n.id=`m11-card-${e.id}`,n.style.borderColor=e.color+"44";const r=t.surgeIds.includes(e.id)?'<span style="font-size:9px;background:#ff4444;color:#fff;border-radius:4px;padding:1px 5px;margin-left:4px;">2× SURGE</span>':"";n.innerHTML=`
      ...
      `,$.appendChild(n);const s=document.createElement("style");s.textContent=`#m11-sl-${e.id}::-webkit-slider-thumb{background:${e.color}}#m11-sl-${e.id}::-moz-range-thumb{background:${e.color}}`,document.head.appendChild(s);const l=n.querySelector(`#m11-sl-${e.id}`);l.addEventListener("input",()=>V(e.id,parseInt(l.value)))})}function j(){const t=A(),o=B(),e=Math.min(t/o,1)*100,n=t>o,r=document.getElementById("m11-pool-bar"),s=document.getElementById("m11-pool-text"),l=document.getElementById("m11-pool-limit");r&&(r.style.width=e+"%",r.style.background=n?"#ff4444":e>80?"#ffb347":"#2ecc71"),s&&(s.textContent=`${t} / ${o} units${n?" — OVER LIMIT!":""}`,s.style.color=n?"#ff4444":"#fff"),l&&(l.textContent=String(o))}function H(t){const o=u.find(k=>k.id===t);if(!o)return;const e=f[t]||0,n=S(e,o.needs),r=n>=1?"#2ecc71":n>=.7?"#f9ca24":"#ff4444",s=o.priority==="CRITICAL",l=e===0,y=document.getElementById(`m11-val-${t}`),z=document.getElementById(`m11-hbar-${t}`),D=document.getElementById(`m11-hap-${t}`),W=document.getElementById(`m11-warn-${t}`),w=document.getElementById(`m11-card-${t}`),F=document.getElementById(`m11-sl-${t}`);if(y&&(y.textContent=String(e)),z&&(z.style.width=n*100+"%",z.style.background=r),D&&(D.textContent=ie(n)),W&&(W.style.display=s&&l?"block":"none"),w&&(s&&l?(w.classList.add("m11-card-pulse"),w.style.borderColor="#ff444488"):(w.classList.remove("m11-card-pulse"),w.style.borderColor=o.color+"44")),F){const k=e/60*100;F.style.background=`linear-gradient(to right,${o.color} ${k}%,#333 ${k}%)`}}function V(t,o){const e=f[t]||0,n=A()-e,r=Math.min(60,B()-n),s=Math.max(0,Math.min(o,r));if(s!==o){const l=document.getElementById(`m11-sl-${t}`);l&&(l.value=String(s))}f[t]=s,H(t),j(),I.click()}function _(){v=30,O(),m=setInterval(()=>{v--,O(),v<=0&&(clearInterval(m),m=null,U())},1e3)}function O(){const t=document.getElementById("m11-timer"),o=document.getElementById("m11-timer-bar");t&&(t.textContent=v+"s"),o&&(o.style.width=v/30*100+"%")}function N(){Y(),P();const t=document.getElementById("m11-round-label");if(t){const o=E[c];t.textContent=`Round ${c+1}/3 — ${o.label}`,t.style.color=c===2?"#ff6b6b":c===1?"#ffb347":"#fff"}h.setLeft(`Score: ${p}`),h.setRight(`Round ${c+1} / 3`),j(),u.forEach(o=>H(o.id)),d.disabled=!1,d.style.opacity="1",_()}function U(){m&&(clearInterval(m),m=null),d.disabled=!0,d.style.opacity="0.5";const t=re(u,f);p+=t,h.setLeft(`Score: ${p}`),u.filter(e=>e.priority==="CRITICAL"&&(f[e.id]||0)===0).length>0?(M=!0,I.fail()):I.coin(),u.forEach((e,n)=>{setTimeout(()=>{const r=S(f[e.id]||0,e.needs),s=r>=1?"😊":r>=.7?"😐":"😡",l=document.getElementById(`m11-card-${e.id}`);if(!l)return;const y=document.createElement("div");y.className="m11-floater",y.textContent=s,l.appendChild(y),setTimeout(()=>y.remove(),1200)},n*90)}),setTimeout(()=>q(t,c>=2),1400)}function q(t,o){const e=J(i,{title:`◈ Round ${c+1} Complete`,color:"#ffb347"}),n=u.filter(s=>s.priority==="CRITICAL"&&(f[s.id]||0)===0).length*50;e.body.innerHTML=`
    ...
    `,r.textContent=o?"See Final Results":`Round ${c+2}: ${E[c+1].label} →`,r.addEventListener("click",()=>{e.remove(),o?K():(c++,N())}),e.body.appendChild(r)}function K(){let t=M?0:1;p>500&&(t=2),p>750&&(t=3);const o=Math.floor(p/10),e=["Needs Work","Bandwidth Manager","Efficient Allocator","QoS Master!"];t>=2?I.win():I.fail(),X(i,{stars:t,title:e[t],lines:[`Total score: ${p}`,p>750?"Near-optimal allocation every round!":p>500?"Good balance of priorities!":"Try allocating more to critical services.",t<3?"Score >750 for ⭐⭐⭐":"All services optimally served! 🏆"],coins:o,color:"#ffb347",onContinue:(n,r)=>{G(),n!=="retry"?g(r,o):se(a,x,g)}})}function G(){m&&(clearInterval(m),m=null),i.remove()}Z(i,{concept:b("m11.concept"),detail:b("m11.banner"),color:"#e17055"}),ee(i,{emoji:"💰",title:b("m11.title"),concept:b("m11.concept"),howto:b("m11.howto"),color:"#e17055",onStart:()=>{N()}})}export{se as launch};
