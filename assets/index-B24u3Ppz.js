import{a as F,s as K,b as V,d as _}from"./main-vhkw1vPQ.js";import{s as T}from"./sfx-DMPbFRfs.js";import"./modulepreload-polyfill-B5Qt9EMX.js";const W=[{id:"medical",emoji:"🚑",label:"EMERGENCY",priority:4,color:"#ff6b6b"},{id:"satellite",emoji:"🛰",label:"CRITICAL",priority:3,color:"#c9b6ff"},{id:"gifts",emoji:"🎁",label:"NORMAL",priority:2,color:"#ffd700"},{id:"pizza",emoji:"🍕",label:"LOW",priority:1,color:"#ff9944"}];let J=0;function L(){return{...W[Math.floor(Math.random()*W.length)],uid:++J}}function Z(){if(document.getElementById("m3-styles"))return;const x=document.createElement("style");x.id="m3-styles",x.textContent=`
  ...
  `,document.head.appendChild(x)}function re(x,ee,S){Z();const r=document.createElement("div");r.style.cssText=`position:fixed;inset:0;background:#05001a;z-index:10;
    font-family:'Space Mono',monospace,sans-serif;overflow:hidden;`,x.appendChild(r);const h=document.createElement("canvas");h.style.cssText="position:absolute;inset:0;pointer-events:none;",r.appendChild(h);function Y(){const n=window.devicePixelRatio||1;h.width=r.clientWidth*n,h.height=r.clientHeight*n,h.style.width=r.clientWidth+"px",h.style.height=r.clientHeight+"px";const o=h.getContext("2d");o.setTransform(n,0,0,n,0,0),o.clearRect(0,0,r.clientWidth,r.clientHeight);for(let e=0;e<120;e++){const t=Math.random()*r.clientWidth,a=Math.random()*r.clientHeight,i=Math.random()*1.5+.3;o.beginPath(),o.arc(t,a,i,0,Math.PI*2),o.fillStyle=`rgba(255,255,255,${.3+Math.random()*.5})`,o.fill()}}Y();let s=[],c=[],v=0,M=90,m=0,z=0,k=!1,b=8,E=null,f=null;const y=F(r,{color:"#c9b6ff"});y.leftEl.style.cursor="pointer",y.leftEl.title="Quit",y.leftEl.addEventListener("click",()=>I(!0));function P(){y.setLeft("◀ Back"),y.setCenter(`🚀 ${m}/10 &nbsp;|&nbsp; ✅ ${z} correct &nbsp;|&nbsp; ⭐ ${v}pts`),y.setRight(`⏱ ${M}s`)}const g=document.createElement("div");g.className="m3-col m3-col-left",r.appendChild(g);const u=document.createElement("div");u.className="m3-col m3-col-right",r.appendChild(u);const $=document.createElement("div");$.className="m3-pad",$.innerHTML=`
    ...
    <div style="font-size:10px;color:#8aa6b4;margin-top:4px;letter-spacing:1px;">
    ...
    </div>`,r.appendChild($);function R(n,o,e){const t=document.createElement("div");t.className="m3-card",t.draggable=!0,t.style.cssText+=`background:${n.color}22;border-color:${n.color}55;`,t.innerHTML=`
      ...
      <div class="m3-badge" style="color:${n.color}">P${n.priority}</div>`,t.addEventListener("dragstart",p=>{E=o,f=e,t.classList.add("dragging"),p.dataTransfer.effectAllowed="move"}),t.addEventListener("dragend",()=>t.classList.remove("dragging"));let a=null,i=0,d=0;return t.addEventListener("touchstart",p=>{const l=p.touches[0];E=o,f=e,i=l.clientX-t.getBoundingClientRect().left,d=l.clientY-t.getBoundingClientRect().top,a=t.cloneNode(!0),a.style.cssText+=`position:fixed;z-index:999;opacity:0.85;pointer-events:none;
        width:${t.offsetWidth}px;left:${l.clientX-i}px;top:${l.clientY-d}px;`,document.body.appendChild(a)},{passive:!0}),t.addEventListener("touchmove",p=>{p.preventDefault();const l=p.touches[0];a&&(a.style.left=l.clientX-i+"px",a.style.top=l.clientY-d+"px")},{passive:!1}),t.addEventListener("touchend",p=>{a&&(a.remove(),a=null);const l=p.changedTouches[0],D=document.elementFromPoint(l.clientX,l.clientY),j=D&&D.closest(".m3-col");j===u?C("queue",null):j===g&&C("incoming",null)}),t}function w(){g.innerHTML="",u.innerHTML="";const n=document.createElement("div");n.className="m3-col-title",n.textContent=`📡 INCOMING (${s.length}/5)`,g.appendChild(n);const o=document.createElement("div");if(o.className="m3-col-title",o.textContent="🚀 LAUNCH QUEUE",u.appendChild(o),s.length===0){const t=document.createElement("div");t.className="m3-drop-zone",t.textContent="AWAITING ROCKETS…",g.appendChild(t)}else s.forEach((t,a)=>g.appendChild(R(t,"incoming",a)));const e=document.createElement("div");e.className="m3-drop-zone",e.style.minHeight=c.length===0?"80px":"44px",e.textContent=c.length===0?"DROP ROCKETS HERE":"+",e.addEventListener("dragover",t=>{t.preventDefault(),e.classList.add("drag-over")}),e.addEventListener("dragleave",()=>e.classList.remove("drag-over")),e.addEventListener("drop",t=>{t.preventDefault(),e.classList.remove("drag-over"),C("queue",null)}),u.appendChild(e),c.forEach((t,a)=>{const i=R(t,"queue",a);i.addEventListener("dragover",d=>{d.preventDefault(),i.style.borderTopColor="#46f0c0"}),i.addEventListener("dragleave",()=>{i.style.borderTopColor=t.color+"55"}),i.addEventListener("drop",d=>{d.preventDefault(),i.style.borderTopColor=t.color+"55",C("queue",a)}),u.appendChild(i)}),u.addEventListener("dragover",t=>{t.preventDefault()}),u.addEventListener("drop",t=>{t.preventDefault(),C("queue",null)}),P(),document.getElementById("m3-cd").textContent=b+"s"}function C(n,o){if(E===null)return;let e;if(E==="incoming"){if(f===null||f>=s.length)return;e=s.splice(f,1)[0]}else{if(f===null||f>=c.length)return;e=c.splice(f,1)[0]}n==="queue"?o===null?c.push(e):c.splice(o,0,e):s.push(e),E=null,f=null,T.swipe(),w()}function q(){if(c.length===0)return;const n=Math.max(...c.map(t=>t.priority)),o=c.shift(),e=o.priority===n;m++,e?(z++,v+=50,T.launch(),X(o.emoji,o.color),N("green"),H(`✅ Correct! P${o.priority} launched first`,"#46f0c0")):(v=Math.max(0,v-20),T.fail(),N("red"),H(`❌ Wrong order! P${n} should launch first`,"#ff6b6b")),b=8,w(),U()}function H(n,o){const e=document.createElement("div");e.style.cssText=`position:absolute;top:60px;left:50%;transform:translateX(-50%);
      ...
      animation:popIn 0.3s ease both;`,e.textContent=n,r.appendChild(e),setTimeout(()=>e.remove(),1800)}function X(n,o){const e=document.createElement("div");e.style.cssText=`position:absolute;bottom:145px;left:50%;transform:translateX(-50%);
      font-size:36px;pointer-events:none;z-index:70;animation:m3-launch 0.7s ease-out forwards;`,e.textContent=n,r.appendChild(e),setTimeout(()=>e.remove(),750);for(let t=0;t<12;t++){const a=document.createElement("div"),i=-90+(Math.random()-.5)*60,d=60+Math.random()*80,p=Math.round(Math.cos(i*Math.PI/180)*d),l=Math.round(Math.sin(i*Math.PI/180)*d);a.style.cssText=`position:absolute;bottom:145px;left:50%;
        ...
        --dx:${p}px;--dy:${l}px;
        animation:m3-particle 0.6s ease-out ${t*30}ms forwards;`,r.appendChild(a),setTimeout(()=>a.remove(),700)}}function N(n){r.style.animation="",r.offsetWidth,n==="red"?(r.style.animation="m3-shake-screen 0.4s ease",setTimeout(()=>{r.style.animation=""},420)):(r.style.boxShadow="inset 0 0 40px #46f0c080",setTimeout(()=>{r.style.boxShadow=""},600))}function O(){const n=Math.min(3,5-s.length);for(let e=0;e<n;e++)s.push(L());const o=document.createElement("div");o.className="m3-surge",o.textContent="⚡ SURGE! ⚡",r.appendChild(o),setTimeout(()=>o.remove(),1800),T.boom(),w()}function U(){m>=10&&I(!1)}function I(n){if(k)return;if(k=!0,clearInterval(Q),clearInterval(B),clearInterval(G),n){S(0,0),r.remove();return}const o=m>0?z/m:0;let e=0;m>=5&&(e=1),m>=8&&o>.6&&(e=2),m>=10&&o>.8&&(e=3);const t=e*30+Math.floor(v/10);_(r,{stars:e,title:e===3?"Perfect Mission! 🚀":e===2?"Great Work!":"Mission Done",lines:[`Rockets launched: ${m}`,`Correct order: ${Math.round(o*100)}%`,`Score: ${v} pts`,'<br><strong style="color:#46f0c0">You just built a priority queue — the same algorithm hospitals,<br>911 centers, and internet routers use every second! 🌐</strong>'],coins:t,color:"#c9b6ff",onContinue:()=>{S(e,t),r.remove()}})}function A(){s.length<5&&(s.push(L()),w())}const B=setInterval(A,3e3),G=setInterval(O,2e4),Q=setInterval(()=>{if(k)return;M--,b--,P();const n=document.getElementById("m3-cd");n&&(n.textContent=b+"s"),b<=0&&(b=8,q()),M<=0&&I(!1)},1e3);K(r,{concept:"Priority Queues & Scheduling",detail:"Networks prioritise critical traffic (VoIP, video) over bulk data. This is called QoS — Quality of Service.",color:"#ff6b35"}),V(r,{emoji:"🚀",title:"Rocket Launch",concept:"Priority Queues decide which task runs FIRST. Hospitals get ambulances before pizza deliveries. Networks send emergency data before cat videos.",howto:`Rockets arrive on the LEFT with priorities P1–P4.
...
Wrong order = points deducted.`,color:"#c9b6ff",onStart:()=>{s.push(L()),s.push(L()),w()}})}export{re as launch};
