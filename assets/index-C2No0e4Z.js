import{a as Q,t as b,e as J,d as X,s as Z,b as ee}from"./main-BAQBCe3n.js";import{s as I}from"./sfx-DMPbFRfs.js";import"./modulepreload-polyfill-B5Qt9EMX.js";const te=[{id:"hospital",emoji:"🚑",name:"Hospital",priority:"CRITICAL",needs:40,color:"#ff6b6b"},{id:"fire",emoji:"🚒",name:"Fire Station",priority:"CRITICAL",needs:30,color:"#ff9f43"},{id:"school",emoji:"🏫",name:"School",priority:"HIGH",needs:25,color:"#ffd700"},{id:"news",emoji:"📡",name:"News Station",priority:"HIGH",needs:20,color:"#74b9ff"},{id:"residential",emoji:"🏠",name:"Residential",priority:"MEDIUM",needs:35,color:"#a29bfe"},{id:"mall",emoji:"🛒",name:"Mall",priority:"MEDIUM",needs:25,color:"#fd79a8"},{id:"gaming",emoji:"🎮",name:"Gaming Server",priority:"LOW",needs:50,color:"#55efc4"},{id:"streaming",emoji:"📺",name:"Streaming",priority:"LOW",needs:45,color:"#636e72"}],oe={CRITICAL:4,HIGH:3,MEDIUM:2,LOW:1},E=[{label:"Normal Operations",pool:150,surgeIds:[]},{label:"SURGE EVENT",pool:150,surgeIds:["hospital","fire"]},{label:"BLACKOUT",pool:80,surgeIds:[]}];function ne(){if(document.getElementById("m11-styles"))return;const l=document.createElement("style");l.id="m11-styles",l.textContent=`
  ...
  `,document.head.appendChild(l)}function S(l,g){return l>=g?1:l>=g*.7?.7:.3}function ie(l){return l>=1?"😊":l>=.7?"😐":"😤"}function re(l,g){let y=0;for(const i of l){const h=S(g[i.id]||0,i.needs);i.priority==="CRITICAL"&&(g[i.id]||0)===0&&(y-=50),y+=h*100*oe[i.priority]}return Math.round(y)}function le(l,g,y){ne();const i=document.createElement("div");i.style.cssText=`
    position:fixed;inset:0;background:#1a0800;
    font-family:'Space Mono',monospace,sans-serif;
    overflow-y:auto;z-index:10;color:#fff;
  `,l.appendChild(i);const h=Q(i,{color:"#ffb347"}),C=document.createElement("button");C.style.cssText=`
  ...
  `,C.textContent=b("btn.back"),C.addEventListener("click",()=>{G(),y(0,0)}),i.appendChild(C);let c=0,p=0,v=30,m=null,M=!1,f={},u=[];function Y(){const t=E[c];u=te.map(o=>({...o,needs:t.surgeIds.includes(o.id)?o.needs*2:o.needs})),f={},u.forEach(o=>{f[o.id]=0})}function B(){return E[c].pool}function A(){return Object.values(f).reduce((t,o)=>t+o,0)}const T=document.createElement("div");T.style.cssText="text-align:center;padding:56px 16px 6px;",T.innerHTML=`
  ...
  `,i.appendChild($);const a=document.createElement("button");a.style.cssText=`
  ...
  `,a.textContent="CONFIRM ALLOCATION",a.addEventListener("mouseenter",()=>{a.style.transform="scale(1.04)",a.style.boxShadow="0 6px 28px rgba(255,160,50,0.6)"}),a.addEventListener("mouseleave",()=>{a.style.transform="scale(1)",a.style.boxShadow="0 4px 20px rgba(255,160,50,0.4)"}),a.addEventListener("click",U),i.appendChild(a);function P(){$.innerHTML="";const t=E[c],o={CRITICAL:"#ff6b6b",HIGH:"#ffd700",MEDIUM:"#a29bfe",LOW:"#55efc4"};u.forEach(e=>{const n=document.createElement("div");n.className="m11-bidder-card",n.id=`m11-card-${e.id}`,n.style.borderColor=e.color+"44";const s=t.surgeIds.includes(e.id)?'<span style="font-size:9px;background:#ff4444;color:#fff;border-radius:4px;padding:1px 5px;margin-left:4px;">2× SURGE</span>':"";n.innerHTML=`
      ...
      `,$.appendChild(n);const r=document.createElement("style");r.textContent=`#m11-sl-${e.id}::-webkit-slider-thumb{background:${e.color}}#m11-sl-${e.id}::-moz-range-thumb{background:${e.color}}`,document.head.appendChild(r);const d=n.querySelector(`#m11-sl-${e.id}`);d.addEventListener("input",()=>V(e.id,parseInt(d.value)))})}function j(){const t=A(),o=B(),e=Math.min(t/o,1)*100,n=t>o,s=document.getElementById("m11-pool-bar"),r=document.getElementById("m11-pool-text"),d=document.getElementById("m11-pool-limit");s&&(s.style.width=e+"%",s.style.background=n?"#ff4444":e>80?"#ffb347":"#2ecc71"),r&&(r.textContent=`${t} / ${o} units${n?" — OVER LIMIT!":""}`,r.style.color=n?"#ff4444":"#fff"),d&&(d.textContent=String(o))}function H(t){const o=u.find(k=>k.id===t);if(!o)return;const e=f[t]||0,n=S(e,o.needs),s=n>=1?"#2ecc71":n>=.7?"#f9ca24":"#ff4444",r=o.priority==="CRITICAL",d=e===0,x=document.getElementById(`m11-val-${t}`),z=document.getElementById(`m11-hbar-${t}`),D=document.getElementById(`m11-hap-${t}`),W=document.getElementById(`m11-warn-${t}`),w=document.getElementById(`m11-card-${t}`),F=document.getElementById(`m11-sl-${t}`);if(x&&(x.textContent=String(e)),z&&(z.style.width=n*100+"%",z.style.background=s),D&&(D.textContent=ie(n)),W&&(W.style.display=r&&d?"block":"none"),w&&(r&&d?(w.classList.add("m11-card-pulse"),w.style.borderColor="#ff444488"):(w.classList.remove("m11-card-pulse"),w.style.borderColor=o.color+"44")),F){const k=e/60*100;F.style.background=`linear-gradient(to right,${o.color} ${k}%,#333 ${k}%)`}}function V(t,o){const e=f[t]||0,n=A()-e,s=Math.min(60,B()-n),r=Math.max(0,Math.min(o,s));if(r!==o){const d=document.getElementById(`m11-sl-${t}`);d&&(d.value=String(r))}f[t]=r,H(t),j(),I.click()}function _(){v=30,O(),m=setInterval(()=>{v--,O(),v<=0&&(clearInterval(m),m=null,U())},1e3)}function O(){const t=document.getElementById("m11-timer"),o=document.getElementById("m11-timer-bar");t&&(t.textContent=v+"s"),o&&(o.style.width=v/30*100+"%")}function N(){Y(),P();const t=document.getElementById("m11-round-label");if(t){const o=E[c];t.textContent=`Round ${c+1}/3 — ${o.label}`,t.style.color=c===2?"#ff6b6b":c===1?"#ffb347":"#fff"}h.setLeft(`Score: ${p}`),h.setRight(`Round ${c+1} / 3`),j(),u.forEach(o=>H(o.id)),a.disabled=!1,a.style.opacity="1",_()}function U(){m&&(clearInterval(m),m=null),a.disabled=!0,a.style.opacity="0.5";const t=re(u,f);p+=t,h.setLeft(`Score: ${p}`),u.filter(e=>e.priority==="CRITICAL"&&(f[e.id]||0)===0).length>0?(M=!0,I.fail()):I.coin(),u.forEach((e,n)=>{setTimeout(()=>{const s=S(f[e.id]||0,e.needs),r=s>=1?"😊":s>=.7?"😐":"😡",d=document.getElementById(`m11-card-${e.id}`);if(!d)return;const x=document.createElement("div");x.className="m11-floater",x.textContent=r,d.appendChild(x),setTimeout(()=>x.remove(),1200)},n*90)}),setTimeout(()=>q(t,c>=2),1400)}function q(t,o){const e=J(i,{title:`◈ Round ${c+1} Complete`,color:"#ffb347"}),n=u.filter(r=>r.priority==="CRITICAL"&&(f[r.id]||0)===0).length*50;e.body.innerHTML=`
