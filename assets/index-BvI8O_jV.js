import{a as K,b as Q,s as J}from"./main-BK2OWqfb.js";import{s as w}from"./sfx-DMPbFRfs.js";import"./modulepreload-polyfill-B5Qt9EMX.js";const X=[{id:"hospital",emoji:"🚑",name:"Hospital",priority:"CRITICAL",needs:40,color:"#ff6b6b"},{id:"fire",emoji:"🚒",name:"Fire Station",priority:"CRITICAL",needs:30,color:"#ff9f43"},{id:"school",emoji:"🏫",name:"School",priority:"HIGH",needs:25,color:"#ffd700"},{id:"news",emoji:"📡",name:"News Station",priority:"HIGH",needs:20,color:"#74b9ff"},{id:"residential",emoji:"🏠",name:"Residential",priority:"MEDIUM",needs:35,color:"#a29bfe"},{id:"mall",emoji:"🛒",name:"Mall",priority:"MEDIUM",needs:25,color:"#fd79a8"},{id:"gaming",emoji:"🎮",name:"Gaming Server",priority:"LOW",needs:50,color:"#55efc4"},{id:"streaming",emoji:"📺",name:"Streaming",priority:"LOW",needs:45,color:"#636e72"}],Z={CRITICAL:4,HIGH:3,MEDIUM:2,LOW:1},I=[{label:"Normal Operations",pool:150,surgeIds:[]},{label:"SURGE EVENT",pool:150,surgeIds:["hospital","fire"]},{label:"BLACKOUT",pool:80,surgeIds:[]}];function ee(){if(document.getElementById("m11-styles"))return;const l=document.createElement("style");l.id="m11-styles",l.textContent=`
  ...
  `,document.head.appendChild(l)}function z(l,g){return l>=g?1:l>=g*.7?.7:.3}function te(l){return l>=1?"😊":l>=.7?"😐":"😤"}function oe(l,g){let y=0;for(const i of l){const b=z(g[i.id]||0,i.needs);i.priority==="CRITICAL"&&(g[i.id]||0)===0&&(y-=50),y+=b*100*Z[i.priority]}return Math.round(y)}function se(l,g,y){ee();const i=document.createElement("div");i.style.cssText=`
    position:fixed;inset:0;background:#1a0800;
    font-family:'Space Mono',monospace,sans-serif;
    overflow-y:auto;z-index:10;color:#fff;
  `,l.appendChild(i);const b=K(i,{color:"#ffb347"}),E=document.createElement("button");E.style.cssText=`
  ...
  `,E.textContent="← Missions",E.addEventListener("click",()=>{U(),y(0,0)}),i.appendChild(E);let c=0,p=0,h=30,m=null,M=!1,f={},u=[];function F(){const t=I[c];u=X.map(o=>({...o,needs:t.surgeIds.includes(o.id)?o.needs*2:o.needs})),f={},u.forEach(o=>{f[o.id]=0})}function S(){return I[c].pool}function B(){return Object.values(f).reduce((t,o)=>t+o,0)}const k=document.createElement("div");k.style.cssText="text-align:center;padding:56px 16px 6px;",k.innerHTML=`
  ...
  `,i.appendChild(k);const T=document.createElement("div");T.style.cssText="padding:8px 20px 4px;max-width:860px;margin:0 auto;",T.innerHTML=`
  ...
  `,i.appendChild(T);const L=document.createElement("div");L.style.cssText="padding:4px 20px 6px;max-width:860px;margin:0 auto;",L.innerHTML=`
  ...
  `,i.appendChild(L);const C=document.createElement("div");C.style.cssText=`
  ...
  `,a.textContent="CONFIRM ALLOCATION",a.addEventListener("mouseenter",()=>{a.style.transform="scale(1.04)",a.style.boxShadow="0 6px 28px rgba(255,160,50,0.6)"}),a.addEventListener("mouseleave",()=>{a.style.transform="scale(1)",a.style.boxShadow="0 4px 20px rgba(255,160,50,0.4)"}),a.addEventListener("click",N),i.appendChild(a);function Y(){C.innerHTML="";const t=I[c],o={CRITICAL:"#ff6b6b",HIGH:"#ffd700",MEDIUM:"#a29bfe",LOW:"#55efc4"};u.forEach(e=>{const n=document.createElement("div");n.className="m11-bidder-card",n.id=`m11-card-${e.id}`,n.style.borderColor=e.color+"44";const s=t.surgeIds.includes(e.id)?'<span style="font-size:9px;background:#ff4444;color:#fff;border-radius:4px;padding:1px 5px;margin-left:4px;">2× SURGE</span>':"";n.innerHTML=`
      ...
      `,C.appendChild(n);const r=document.createElement("style");r.textContent=`#m11-sl-${e.id}::-webkit-slider-thumb{background:${e.color}}#m11-sl-${e.id}::-moz-range-thumb{background:${e.color}}`,document.head.appendChild(r);const d=n.querySelector(`#m11-sl-${e.id}`);d.addEventListener("input",()=>P(e.id,parseInt(d.value)))})}function A(){const t=B(),o=S(),e=Math.min(t/o,1)*100,n=t>o,s=document.getElementById("m11-pool-bar"),r=document.getElementById("m11-pool-text"),d=document.getElementById("m11-pool-limit");s&&(s.style.width=e+"%",s.style.background=n?"#ff4444":e>80?"#ffb347":"#2ecc71"),r&&(r.textContent=`${t} / ${o} units${n?" — OVER LIMIT!":""}`,r.style.color=n?"#ff4444":"#fff"),d&&(d.textContent=String(o))}function H(t){const o=u.find($=>$.id===t);if(!o)return;const e=f[t]||0,n=z(e,o.needs),s=n>=1?"#2ecc71":n>=.7?"#f9ca24":"#ff4444",r=o.priority==="CRITICAL",d=e===0,x=document.getElementById(`m11-val-${t}`),R=document.getElementById(`m11-hbar-${t}`),G=document.getElementById(`m11-hap-${t}`),D=document.getElementById(`m11-warn-${t}`),v=document.getElementById(`m11-card-${t}`),W=document.getElementById(`m11-sl-${t}`);if(x&&(x.textContent=String(e)),R&&(R.style.width=n*100+"%",R.style.background=s),G&&(G.textContent=te(n)),D&&(D.style.display=r&&d?"block":"none"),v&&(r&&d?(v.classList.add("m11-card-pulse"),v.style.borderColor="#ff444488"):(v.classList.remove("m11-card-pulse"),v.style.borderColor=o.color+"44")),W){const $=e/60*100;W.style.background=`linear-gradient(to right,${o.color} ${$}%,#333 ${$}%)`}}function P(t,o){const e=f[t]||0,n=B()-e,s=Math.min(60,S()-n),r=Math.max(0,Math.min(o,s));if(r!==o){const d=document.getElementById(`m11-sl-${t}`);d&&(d.value=String(r))}f[t]=r,H(t),A(),w.click()}function V(){h=30,j(),m=setInterval(()=>{h--,j(),h<=0&&(clearInterval(m),m=null,N())},1e3)}function j(){const t=document.getElementById("m11-timer"),o=document.getElementById("m11-timer-bar");t&&(t.textContent=h+"s"),o&&(o.style.width=h/30*100+"%")}function O(){F(),Y();const t=document.getElementById("m11-round-label");if(t){const o=I[c];t.textContent=`Round ${c+1}/3 — ${o.label}`,t.style.color=c===2?"#ff6b6b":c===1?"#ffb347":"#fff"}b.setLeft(`Score: ${p}`),b.setRight(`Round ${c+1} / 3`),A(),u.forEach(o=>H(o.id)),a.disabled=!1,a.style.opacity="1",V()}function N(){m&&(clearInterval(m),m=null),a.disabled=!0,a.style.opacity="0.5";const t=oe(u,f);p+=t,b.setLeft(`Score: ${p}`),u.filter(e=>e.priority==="CRITICAL"&&(f[e.id]||0)===0).length>0?(M=!0,w.fail()):w.coin(),u.forEach((e,n)=>{setTimeout(()=>{const s=z(f[e.id]||0,e.needs),r=s>=1?"😊":s>=.7?"😐":"😡",d=document.getElementById(`m11-card-${e.id}`);if(!d)return;const x=document.createElement("div");x.className="m11-floater",x.textContent=r,d.appendChild(x),setTimeout(()=>x.remove(),1200)},n*90)}),setTimeout(()=>_(t,c>=2),1400)}function _(t,o){const e=Q(i,{title:`◈ Round ${c+1} Complete`,color:"#ffb347"}),n=u.filter(r=>r.priority==="CRITICAL"&&(f[r.id]||0)===0).length*50;e.body.innerHTML=`
    ...
    `,s.textContent=o?"See Final Results":`Round ${c+2}: ${I[c+1].label} →`,s.addEventListener("click",()=>{e.remove(),o?q():(c++,O())}),e.body.appendChild(s)}function q(){let t=M?0:1;p>500&&(t=2),p>750&&(t=3);const o=Math.floor(p/10),e=["Needs Work","Bandwidth Manager","Efficient Allocator","QoS Master!"];t>=2?w.win():w.fail(),J(i,{stars:t,title:e[t],lines:[`Total score: ${p}`,p>750?"Near-optimal allocation every round!":p>500?"Good balance of priorities!":"Try allocating more to critical services.",t<3?"Score >750 for ⭐⭐⭐":"All services optimally served! 🏆"],coins:o,color:"#ffb347",onContinue:n=>{U(),y(n,o)}})}function U(){m&&(clearInterval(m),m=null),i.remove()}O()}export{se as launch};
